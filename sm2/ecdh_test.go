...
func TestECDH(t *testing.T) {
	aliceKey, err := P256().GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	bobKey, err := P256().GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}

	alicePubKey, err := P256().NewPublicKey(aliceKey.PublicKey().Bytes())
	if err != nil {
		t.Error(err)
	}
	...
	alicePrivKey, err := P256().NewPrivateKey(aliceKey.Bytes())
	if err != nil {
		t.Error(err)
	}
...
func TestSM2MQV(t *testing.T) {
	aliceSKey, err := P256().GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	aliceEKey, err := P256().GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}

	bobSKey, err := P256().GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	bobEKey, err := P256().GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSM2SharedKeyVectors(t *testing.T) {
	...
	for i, v := range vectors {
		aliceSKey, err := P256().NewPrivateKey(hexDecode(t, v.LocalStaticPriv))
		if err != nil {
			t.Fatal(err)
		}
		aliceEKey, err := P256().NewPrivateKey(hexDecode(t, v.LocalEphemeralPriv))
		if err != nil {
			t.Fatal(err)
		}
		bobSKey, err := P256().NewPrivateKey(hexDecode(t, v.RemoteStaticPriv))
		if err != nil {
			t.Fatal(err)
		}
		bobEKey, err := P256().NewPrivateKey(hexDecode(t, v.RemoteEphemeralPriv))
		if err != nil {
			t.Fatal(err)
		}
...
func benchmarkAllCurves(b *testing.B, f func(b *testing.B, curve Curve)) {
	b.Run("SM2P256", func(b *testing.B) { f(b, P256()) })
}
